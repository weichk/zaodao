...
(function (factory) {
    ...
    var calendar = function(e, options) {
        ...
        this.sameday = options.sameday || 'sameday'; // 开始/结束为同一天
        this.lockedday = options.lockedday || 'lockedday'; // 锁定已经预约的日期
        this.singleDate = options.singleDate || '';
        this.lockDate = options.lockDate || '';
    ...
    calendar.prototype.genDate = function(day) {
        ...
        if ((this.checkIn && writeDay <= this.checkOut && writeDay >= this.checkIn) ||
            ...
            if(this.lockDate) {
                checkDay =this.classDaySelect + ' ' + this.checkedday + ' ' + this.lockedday;
            } else {
    ...
    calendar.prototype.clearSelect = function() {
        ...
        this.e.find('.' + this.sameday).removeClass(this.sameday);
        // this.e.find('.' + this.lockedday).removeClass(this.lockedday);
        this.checkOut = null;
        this.checkIn  = null;
        this.firstGray = false;
        this.markAsGray = false;
    ...
    calendar.prototype.refreshCheckState = function() {
        ...
        if(_this.checkIn && !_this.checkOut && leaveday == enterday) {
            this.doColor = true;
            doColorState = true;
            checkFirst.find('span').html(this.dayText[0] + '<br>' + this.dayText[1]);
